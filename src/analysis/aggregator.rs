...
impl FeatureAggregator {
    ...
    pub fn compute_features(conn: &Connection, window_minutes: i64) -> Result<BlameFeatures> {
        ...
        // 1. Gateway RTT & Loss
        let (gw_p50, gw_p95, gw_loss) = Self::analyze_latency(conn, "icmp", "gateway", &window_start)?;
        features.gw_rtt_p50_ms = gw_p50;
        features.gw_rtt_p95_ms = gw_p95;
        features.gw_loss_pct = gw_loss;

        // 2. WAN RTT & Loss
        let (wan_p50, wan_p95, wan_loss) = Self::analyze_latency(conn, "icmp", "8.8.8.8", &window_start)?; // Using 8.8.8.8 as WAN proxy for now
    ...
    fn analyze_latency(conn: &Connection, probe_type: &str, target: &str, window_start: &str) -> Result<(f64, f64, f64)> {
        ...
        for r in rows {
            if let Ok(v) = r {
                if v >= 0.0 { values.push(v); } // -1 usually means timeout/loss
            }
        }
        ...
        let all_rows = stmt_all.query_map(params![probe_type, target, window_start], |row| row.get::<_, f64>(0))?;
        ...
        let p50 = if len > 0 { valid_latencies[len / 2] } else { 0.0 };
        let p95 = if len > 0 { valid_latencies[(len as f64 * 0.95) as usize] } else { 0.0 };

        Ok((p50, p95, loss_pct))
    }

    fn analyze_app_metric(conn: &Connection, probe_type: &str, window_start: &str) -> Result<(f64, f64)> {
         let mut stmt = conn.prepare(
            "SELECT value FROM measurements 
             WHERE probe_type = ?1 
        ...
        let rows = stmt.query_map(params![probe_type, window_start], |row| row.get::<_, f64>(0))?;
        ...
        for r in rows {
            if let Ok(v) = r {
                total += 1;
                if v < 0.0 { // Assumption: < 0 is failure code
                    fails += 1;
                } else {
                    valid.push(v);
                }
        ...
        let p50 = if !valid.is_empty() { valid[valid.len() / 2] } else { 0.0 };
