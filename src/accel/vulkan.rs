...
impl VulkanBackend {
    ...
    /// # Safety
    ///
    /// - Loads dynamic libraries (`libvulkan.so`).
    /// - Creates raw Vulkan instances and devices.
    pub unsafe fn new() -> Result<Self> {
        ...
        info!(
            "Vulkan compute backend initialized on queue family {}",
            queue_family_index
        );
    ...
    /// # Safety
    ///
    /// - `spirv_code` must be valid SPIR-V bytecode.
    /// - The device must be valid and support the required features.
    ...
    /// # Safety
    ///
    /// - `pipeline`, `pipeline_layout`, and `descriptor_set` must be valid Vulkan handles created from the associated device.
    /// - The `descriptor_set` must match the layout expected by the pipeline.
