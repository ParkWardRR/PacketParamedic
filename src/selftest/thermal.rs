...
pub fn check_throttling() -> Result<ComponentResult> {
    ...
    match output {
        Ok(out) => {
            ...
            if (mask & 0x01) != 0 { details.push("Currently under-voltage"); }
            if (mask & 0x02) != 0 { details.push("Currently frequency capped (CPU)"); }
            if (mask & 0x04) != 0 { details.push("Currently throttled (temp)"); }
            if (mask & 0x10000) != 0 { details.push("Past under-voltage event"); }
            if (mask & 0x20000) != 0 { details.push("Past frequency cap event"); }
            if (mask & 0x40000) != 0 { details.push("Past throttle event"); }
            ...
            let remediation = if status == TestStatus::Fail {
                ...
                Some("System was throttled previously. Ensure stable power/cooling for long tests.".to_string())
            } else {
                None
            };
