...
pub fn check_interfaces() -> Result<Vec<ComponentResult>> {
    ...
    for entry in entries {
        ...
        let is_pcie = entry
            ...
            .join("device")
            .read_link()
            .map(|p| p.to_string_lossy().contains("pci"))
            .unwrap_or(false);
        ...
        if is_pcie && speed_mbps < 5000 && speed_mbps > 0 {
            results.push(ComponentResult {
                ...
                status: TestStatus::Warning,
                details: format!(
                    "PCIe NIC link negotiated at {} Mbps. Verify cable/switch.",
                    speed_mbps
                ),
                remediation: Some(
                    "Check cabling (Cat6a+) and switch port configuration.".to_string(),
                ),
            });
        }
    }
